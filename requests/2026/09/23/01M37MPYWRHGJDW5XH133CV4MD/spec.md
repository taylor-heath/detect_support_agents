...
(8) write the five narrative insights, (9) generate the contents card last, from
the cards that actually rendered.**

The report is fully data-driven: **never invent a value** — if the input does not
contain it, leave it blank or omit the section. The one exception is **Group M —
...
entirely — never rendered empty, and never replaced with a placeholder. **Group M
is the sole exception**: it is written by hand after generation and therefore
renders empty, with its placeholders intact.

### Card 0 — Contents

Single full-width card, first on the page, directly beneath the report title.
...
| Property | Value |
|---|---|
| Entries | One line per group heading, with its cards indented beneath it. Card 0 itself is not listed. |
...
| Group M | Always listed, including its three subheadings, even though it is empty at generation time. |
...
| C2 | **Top 15 brand clusters by URL count** — horizontal bar chart; cluster names are unbounded in length and will not fit as upright labels under fifteen vertical bars. |
...
### Group M — Model Insights

**Completed manually. Never generated, never pre-filled, never omitted.**

This group is the one place in the report where a human writes free prose. The
generator lays out three empty cards, in this order, and stops:

| Card | Title | Content |
|---|---|---|
| M1 | **Introduction** | Empty. Placeholder only. |
| M2 | **Insights** | Empty. Placeholder only. |
| M3 | **Performance** | Empty. Placeholder only. |
...
- Do **not** draft, suggest, summarise or pre-populate any of the three cards.
  Do not carry text across from the Group E narrative cards or from Card 1.
...
- Group M sits after Group E and before Card F, so the manual reading of the model
  follows the generated findings and precedes the methodology note.
...
- Empty states are explicit ("No multi-URL brand clusters were detected in this
  sample."), never fabricated data. Group M is exempt: its three cards are written
...
- Minimum 14 mm per column. The seven subcategory columns of B6, B7 and C4 sit in
...
| Bar orientation | More than eight categories, or any category name over roughly 12 characters, renders horizontally. This covers B1, B5, C2. |
...
| Time axis | At most 12 tick labels on A2. Thin evenly and always keep the first and last date. Never let date labels collide. |
| Pie and donut | No label sits on a segment below 5%. Those values move to a legend carrying name, count and percentage. A3 and B2 both use a legend rather than leader lines. |
...
| Node diagrams | C5 and C11 place labels so that none collide. Where two would, shorten to the cluster stem and list the full names in a caption beneath the diagram. |
